...
u32 FindTitlePID(u64 title_id)
{
    for (unsigned kproc_index = 0; kproc_index < kproc_num; kproc_index++) {
        KCodeSet* curr_codeset = *(KCodeSet**)(kproc_start + kproc_size * kproc_index + kproc_codeset_offset);
        if (curr_codeset != nullptr && curr_codeset->title_id == title_id)
            return *(u32*)(kproc_start + kproc_size * kproc_index + kproc_pid_offset);
    }
    return -1;
}

u32 FindTitlePID(const char* title_name, size_t title_len)
{
    for (unsigned kproc_index = 0; kproc_index < kproc_num; kproc_index++) {
        KCodeSet* curr_codeset = *(KCodeSet**)(kproc_start + kproc_size * kproc_index + kproc_codeset_offset);
        if (curr_codeset != nullptr && memcmp(curr_codeset->title_name, title_name, title_len) == 0)
...
u32 FindCodeOffsetKAddr(KCodeSet* code_set, u32 code_offset)
{
    ...
    while (node != nullptr) {
		...
		if (node == code_set->text_info.last_node) return 0;
        node = node->next;
    }
...
u32 FindDataOffsetKAddr(KCodeSet* code_set, u32 code_offset)
{
    u32 curr_offset = 0;
    ...
    KLinkedListNode* node = code_set->data_info.first_node;
    ...
    while (node != nullptr) {
        ...
        // Current offset: amount of bytes searched so far
        curr_offset += block_size;
		if (node == code_set->data_info.last_node) return 0;
        node = node->next;
    }
